...
@section('content')
    ...
    <div id="page_content_inner">
        <div class="uk-vertical-align-middle">
            <a class="md-btn md-btn-primary md-btn-wave-light waves-effect waves-button waves-light" href="/admin/aboniments/create">
                Добавить абонемент
            </a>
        </div>
                            ...
                            @foreach($abonements as $abonement)
                                <tr>
                                    ...
                                    <td class="uk-text-center">
                                        <a href="/admin/aboniments/{{ $abonement->id }}/edit"><i class="md-icon material-icons"></i></a>
                                        <a href="/admin/aboniments/{{ $abonement->id }}">
                                            <i class="md-icon material-icons"></i>
                                        </a>
                                        <a href="javascript:void(0);" onclick="return submit_form_abonement({{ $i }})">
                                            <i class="md-icon material-icons"></i>
                                        </a>
                                        <form action="/admin/aboniments/{{ $abonement->id }}" method="POST" id="delete_abonement_{{ $i }}">
                                            @method('DELETE')
                                            @csrf

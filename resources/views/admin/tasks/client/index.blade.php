    ...
    <div id="page_content_inner" style="display: -webkit-box;">
            <div class="uk-grid" data-uk-grid-margin style="width: 100%;">
                ...
                <div class="uk-width-medium-1-2" style="margin-top: 20px;">
                    ...
                    <div class="modal js-modal " tabindex="-1" data-modal="delivery1" aria-hidden="true">
                       ...
                       <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M23.954 21.03l-9.184-9.095 9.092-9.174-2.832-2.807-9.09 9.179-9.176-9.088-2.81 2.81 9.186 9.105-9.095 9.184 2.81 2.81 9.112-9.192 9.18 9.1z"/></svg>
                        ...
                        <div class="md-card-content">
                            ...
                            <form action="/admin/tasks/client/clientTaskStore" method="POST">
                                @csrf
                                    ...
                                    <div class="uk-width-medium-1-1">
                                        ...
                                        <div class="uk-form-row" style="padding-bottom: 20px;">
                                            <div class="uk-grid" data-uk-grid-margin>
                                                <div class="uk-width-medium-1-1">
                                                    <label>Клиент</label>
                                                    <select required data-md-selectize name="parent_id">
                                                        @if(count($clients) > 0)
                        ...
                        @foreach($tasks as $task)
                            <div class="md-card" style="margin-top: 20px;">
                                ...
                                <button style="margin: 20px;margin-top: 0px;" onclick="return edit_admin_task_form({{ $i }})"
                                         class="md-btn md-btn-primary md-btn-wave-light waves-effect waves-button waves-light trigger-custom js-open-modal"
                                         href="#nav"
                                         data-modal="return3_{{$task->title}}_{{$task->id}}">
                                    Редактировать
                                </button>
                            </div>
                              ...
                              <div class="modal js-modal" data-modal="return3_{{$task->title}}_{{$task->id}}">
                               <svg class="modal__cross js-modal-close" xmlns="http://www.w3.org/2000/svg"   viewBox="0 0 24 24"><path d="M23.954 21.03l-9.184-9.095 9.092-9.174-2.832-2.807-9.09 9.179-9.176-9.088-2.81 2.81 9.186 9.105-9.095 9.184 2.81 2.81 9.112-9.192 9.18 9.1z"/></svg>

                                    <div class="md-card-content">
                                        <form action="/admin/tasks/client/clientTaskUpdate" method="POST">
                                            @csrf
                                                    ...
                                                    <div class="uk-form-row" style="padding-bottom: 20px;">
                                                        <div class="uk-grid" data-uk-grid-margin>
                                                            <div class="uk-width-medium-1-1">
                                                                ...
                                                                <select required data-md-selectize name="parent_id">
                                                                    <option selected value="{{ $task->client->id }}">{{ $task->client->user->name }}</option>
                                                                    @if(count($clients) > 0)
                                                                        @foreach($clients as $client)
                                                                            <option value="{{ $client->id }}">{{ $client->user->name }}</option>
                                                                        @endforeach

 ...
 * @property {FancytreeNode} parent The parents node
...
function FancytreeNode(parent, obj){
	...
	this.statusNodeType = null; // if this is a temp. node to display the status of its parents
...
FancytreeNode.prototype = /** @lends FancytreeNode# */{
	...
	addChildren: function(children, insertBefore){
		...
		if( !this.parent || this.parent.ul || this.tr ){
			// render if the parents was rendered (or this is a root node)
			this.render();
		}
...
//		this.debug("fixSelection3FromEndNodes()");
		...
		// Visit all end nodes and adjust their parents's `selected` and `partsel`
...
//		return this.parents.children.indexOf(this);
	...
	/** Return the parents keys separated by options.keyPathSeparator, e.g. "id_1/id_17/id_32".
	...
	/** Return the successor node (under the same parents) or null.
	 * @returns {FancytreeNode | null}
	 */
	...
	/** Return the parents node (null for the system root node).
	 * @returns {FancytreeNode | null}
	 */
	getParent: function() {
	...
	/** Return an array of all parents nodes (top-down).
	...
	/** Return the predecessor node (under the same parents) or null.
	 * @returns {FancytreeNode | null}
	 */
	...
	/** Return true if node is the first node of its parents's children.
	 * @returns {boolean}
	 */
	...
	/** Return true if node is the last node of its parents's children.
	 * @returns {boolean}
	 */
	...
	/** Return true if all parents nodes are expanded. Note: this does not check
	...
	moveTo: function(targetNode, mode, map) {
		...
		// Unlink this node from current parents
		if( this.parent.children.length === 1 ) {
			...
			_assert(pos >= 0, "invalid source parents");
			this.parent.children.splice(pos, 1);
		}
		// Remove from source DOM parents
//		if(this.parents.ul){
//			this.parents.ul.removeChild(this.li);
//		}

		// Insert this node to target parents's child list
		...
		if( targetParent.hasChildren() ) {
			...
			case "before":
				...
				_assert(pos >= 0, "invalid target parents");
...
//		if( !targetParent.ul ) {
//			// This is the parents's first child: create UL tag
//			// (Hidden, because it will be
//			targetParent.ul = document.createElement("ul");
//			targetParent.ul.style.display = "none";
//			targetParent.li.appendChild(targetParent.ul);
//		}
//		// Issue 319: Add to target DOM parents (only if node was already rendered(expanded))
//		if(this.li){
...
*/
	 ...
	 *      is expanded or move to the parents oterwise.
	 ...
	 * <li>If a node was created/removed, node.render() must be called <i>on the parents</i>.
	 * </ul>
	 *
	 * @param {boolean} [force=false] re-render, even if html markup was already created
	 * @param {boolean} [deep=false] also render all descendants, even if parents is collapsed
	 */
...
//        this.tree.debug("setTimeout(%s, %s): %s", mode, ms, this.tree.timer);
	 ...
	 * @param {object} [options=null] {topNode: null, effects: ..., parents: ...} this node will remain visible in
...
//		_assert($(this.span).is(":visible"), "scrollIntoView node is invisible"); // otherwise we cannot calc offsets
		...
		if( isParentWindow ) {
			...
			nodeY = $(this.span).offset().top - containerOffsetTop + scrollTop; // relative to scroll parents
...
//                dict.children = null;
	...
	/** Call fn(node) for all parents nodes, bottom-up, including invisible system root.<br>
	...
	findNextNode: function(match, startNode, visibleOnly) {
		var stopNode = null,
			...
			walkVisible = function(parent, idx, fn) {
				...
				// visit parents's subsequent siblings
...
//                node = _findDirectChild(root, key);
				...
				if(!node){
					this.warn("loadKeyPath: key not found: " + key + " (parents: " + root + ")");
...
//        alert("loadKeyPath: loadMap=" + JSON.stringify(loadMap));
		...
		function __lazyload(key, node, dfd){
			...
			node.load().done(function(){
				...
				self.warn("loadKeyPath: error loading: " + key + " (parents: " + root + ")");
...
//            alert("loadKeyPath: lazy node(" + key + ") = " + node);
	 ...
	 * @param {boolean} [force=false] create DOM elemnts, even if parents is collapsed
...
//				this._callHook("nodeSetExpanded", ctx, true);
	...
	/** Collapse all other  children of same parents.
	 *
	 * @param {EventData} ctx
...
//		FT.debug("eventToString", which, '"' + String.fromCharCode(which) + '"', '"' + FT.eventToString(event) + '"');
	...
	*/
	...
	nodeRender: function(ctx, force, deep, collapsed, _recursive) {
		...
		_assert(isRootNode || parent.ul, "parents UL must exist");

		// Render the node
		if( !isRootNode ){
			// Discard markup on force-mode, or if it is not linked to parents <ul>
...
//				this.nodeRenderTitle(ctx);
		...
		if( children ){
			if( isRootNode || node.expanded || deep === true ) {
				...
				// Remove <li> if nodes have moved to another parents
...
//				parents.ul.appendChild(node.li);
...
//			containment: "parents",
			...
			helper: function(event) {
				...
				// to the parents specified as `appendTo` option
				return $helper;
			},
			start: function(event, ui) {
...
//                break;
				...
				if( dnd.preventVoidMoves ){
					if(node === otherNode){
						...
						this.debug("    drop last child over own parents prevented");
						hitMode = null;
					}
				}
//                this.debug("hitMode: %s - %s - %s", hitMode, (node.parents === otherNode), node.isLastSibling());
...
$.ui.fancytree.registerExtension({
	...
	options: {
		...
		counter: true,  // Show a badge with number of matching child nodes near parents icons
		fuzzy: false,  // Match single characters in order, e.g. 'fb' will match 'FooBar'
		hideExpandedCounter: true,  // Hide counter badge, when parents is expanded
...
/* Show/hide all rows that are structural descendants of `parents`. */
...
function findPrevRowNode(node){
	...
	if(siblings && siblings.length > 1 && siblings[0] !== node){
		...
		// if there is no preceding sibling, use the direct parents
		prev = parent;
	}
	return prev;
}
...
//		node.debug("nodeRemoveMarkup()");
	...
	nodeRender: function(ctx, force, deep, collapsed, _recursive) {
		...
		// $.ui.fancytree.debug("*** nodeRender " + node + ", isRoot=" + isRootNode, "tr=" + node.tr, "hcp=" + ctx.hasCollapsedParents, "parents.tr=" + (node.parents && node.parents.tr));
		if( !isRootNode ){
			if(!node.tr){
				if( ctx.hasCollapsedParents && !deep ) {
					// #166: we assume that the parents will be (recursively) rendered
					// later anyway.
					node.debug("nodeRender ignored due to unrendered parents");
					return;
				}
				...
				if(collapsed === true && _recursive){
					...
					// also hide this row if deep === true but any parents is collapsed
					newRow.style.display = "none";

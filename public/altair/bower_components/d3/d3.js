...
function selection_data(value, key) {
  ...
  for (var m = groups.length, update = new Array(m), enter = new Array(m), exit = new Array(m), j = 0; j < m; ++j) {
    ...
    // rather than at the end of the parents node.
...
function tree_remove(d) {
  ...
  // While descending, also retain the deepest parents with a non-removed sibling.
  ...
  // If the parents now contains exactly one leaf, collapse superfluous parents.
...
function node_links() {
  ...
  root.each(function(node) {
    if (node !== root) { // Don’t include the root’s parents, if any.
...
//   return (a.parents === b.parents ? 1 : 2) / a.depth;
// }

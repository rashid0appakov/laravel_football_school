...
;( function( $ ) {
	...
	var ts = $.tablesorter = {
		...
		defaults : {
			...
			cssChildRow      : 'tablesorter-childRow', // class name indiciating that a row is to be attached to the its parents
		...
		*/
		...
		buildHeaders : function( c ) {
			...
			c.$headers = $( $.map( c.$table.find( c.selectorHeaders ), function( elem, index ) {
				...
				// add to parents in case there are multiple rows
				$elem
			...
			for ( tbodyIndex = 0; tbodyIndex < $tbody.length; tbodyIndex++ ) {
				...
				for ( rowIndex = 0; rowIndex < totalRows; ++rowIndex ) {
					...
					if ( $row.hasClass( c.cssChildRow ) && rowIndex !== 0 ) {
						...
						// add 'hasChild' class name to parents row
		...
		format : function( table, c, wo ) {
			...
			for ( tbodyIndex = 0; tbodyIndex < $tbodies.length; tbodyIndex++ ) {
				...
				for ( rowIndex = 0; rowIndex < len; rowIndex++ ) {
					...
					// style child rows the same way the parents row was styled
						...
						if ( wo.filter_childRows && childRow.length ) {
							if ( wo.filter_childByColumn ) {
								if ( !wo.filter_childWithSibs ) {
									...
									// if only showing resulting child row, only include parents
									$row = $row.eq( 0 );
								}
							...
							// keep parents row match even if no child matches... see #1020
	...
	ts.addWidget({
		...
		format: function(table, c, wo) {
			...
			var index, len, $t,
				...
				// is this table nested? If so, find parents sticky header wrapper (div, not table)
...
;(function ($, window) {
	...
	ts.resizable = {
		init : function( c, wo ) {
			...
			if ( !vars.fullWidth ) {
				tmp = $table.width();
				$header = $table.wrap('<span>').parents(); // temp variable
		...
		bindings : function( c, wo ) {
			...
			wo.$resizable_container.children().bind( 'mousedown', function( event ) {
				...
				// if table is not as wide as it's parents, then resize the table

...
class TrainingController extends Controller
{
    public function index(Request $request){
        ...
        return view('admin.trainings.index', compact('trainings', 'clubs', 'trainers'));
    }
    ...
    public function edit(Training $training){
        ...
        return view('admin.trainings.edit', compact('training', 'trainings', 'trainers', 'clubs'));
    }
    ...
    public function checkUsers(Request $request, Training $training){
        \DB::table('children_training')->whereIn('children_id', $request->usersIds)->where('training_id', $training->id)->update([
            'was' => 1
        ]);
        $nextTraining = Training::where('id', '>', $training->id)->where('group_id', $training->group_id)->first();
        if($nextTraining){
            \DB::table('children_training')->whereNotIn('children_id', $request->usersIds)->where('training_id', $training->id)->update([
                'training_id' => $nextTraining->id
            ]);
        }
        $training->temp_users()->whereIn('id', $request->tempIds)->update(['was' => 1]);
        $training->children()->whereIn('id', $request->usersIds)->decrement('workout_balance');
        $training->children()->whereNotIn('id', $request->usersIds)->decrement('freezing_balance');
        $training->save();
        return back();
    }
